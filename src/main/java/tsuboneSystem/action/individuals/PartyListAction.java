...
public class PartyListAction {
	...
	public String index() {
    	...
    	//期限内
    	partyItemOn = tPartyService.findBy_Deadline_GE_Now(dateNow, loginIndividualsDto.memberId);
    	
	    	//(開催日が設定されず、作成日から一ヶ月以内の会議)
    		List<TParty> partyItemNOMeetingDay = tPartyService.findBy_NOMeetingDay_GE(dateNow, loginIndividualsDto.memberId);
			if (partyItemNOMeetingDay.size() != 0 ) {
				partyItemOn.addAll(partyItemNOMeetingDay);
			}
    	
    	//期限外
    	partyItemOff = tPartyService.findBy_Deadline_LE_Now(dateNow, loginIndividualsDto.memberId);
    	
    		//(開催日が設定されず、作成日から一ヶ月以上たった会議)
    		List<TParty> tPartyNOmeetingDay = tPartyService.findBy_NOMeetingDay_LE(dateNow, loginIndividualsDto.memberId);
